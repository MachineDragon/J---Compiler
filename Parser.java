...
public class Parser {
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        ArrayList<TypeName> qIdentifiers = null;
        if (have(IMPLEMENTS)) {
            qIdentifiers = new ArrayList<TypeName>();
            do {
                qIdentifiers.add(qualifiedIdentifier());
            } while (have(COMMA));
        }
        return new JClassDeclaration(line, mods, name, superClass, qIdentifiers, classBody());
    }

    /**
     * Parse an interface declaration and returns an AST for it.
     *
     * <pre>
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods) {
        ...
        return new JInterfaceDeclaration(line, mods, name, qIdentifiers, interfaceBody());
    }
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
            ...
            if (have(VOID)) {
                ...
                memberDecl = new JMethodDeclaration(line, mods, name, type, params, exceptions,
                        body);
            } else {
                type = type();
                if (seeIdentLParen()) {
    ...
    /**
     * Parse an interface member declaration and returns an AST for it.
     *
     * <pre>
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        if (have(CLASS)) {
            ...
            if (have(VOID)) {
                ...
                mustBe(SEMI);
                member = new JMethodDeclaration(line, mods, name, type,
                        params, qIdentifiers, null);
            } else {
                type = type();
                if (seeIdentLParen()) {
                    // Non void method
                    ...
                    // Field
                    member = new JFieldDeclaration(line, mods,
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            if (!hasCatch) {
                if (have(FINALLY)) {
                    return new JTryStatement(line, tryBlock, params, catchBlocks,
                            block());
                } else {
                    reportParserError("Must have a finally block when there is no catch.");
                }
            } else {
                if (have(FINALLY)) {
                    return new JTryStatement(line, tryBlock, params, catchBlocks,
    ...
    /**
     * Parses the initializations for a for loop.
     *
     * <pre>
     ...
     * </pre>
     *
     * @return a list of initialization statements.
     */
    ...
    private SwitchStatementGroup switchBlockStatementGroup() {
        ...
        ArrayList<JStatement> blockstmts = new ArrayList<JStatement>();
        while ((seeLocalVariableDeclaration() || seeStatement())) {
            blockstmts.add(blockStatement());
        }
        return new SwitchStatementGroup(switchLabels, blockstmts);
    }
     ...
     *                              [( ALSHIFT_ASSIGN | AND_ASSIGN | ARSHIFT_ASSIGN | ASSIGN
     *                               | DIV_ASSIGN | LRSHIFT_ASSIGN | MINUS_ASSIGN | OR_ASSIGN
     *                               | PLUS_ASSIGN | REM_ASSIGN | STAR_ASSIGN | XOR_ASSIGN )
    ...
    /**
     * Parse a conditional expression and return an AST for it.
     *
     * <pre>
    ...
    private JExpression conditionalExpression() {
        int line = scanner.token().line();
        JExpression expr = conditionalOrExpression();
        if (have(QUESTION)) {
            JExpression thenPart = expression();
            mustBe(COLON);
            JExpression elsePart = conditionalExpression();
            return new JConditionalExpression(line, expr, thenPart, elsePart);
        }
        return expr;
    }
     ...
     *
     * <pre>
     *   literal ::= CHAR_LITERAL | DOUBLE_LITERAL | FALSE | INT_LITERAL | LONG_LITERAL | NULL
     *             | STRING_LITERAL | TRUE
     * </pre>
     *
    ...
    // Returns true if we are looking at a statement, and false otherwise.
    private boolean seeStatement() {
        if (see(RCURLY) || see(CASE) || see(DEFLT)) {
            return false;
        }
        if (see(LCURLY) || see(IF) || see(FOR) || see(WHILE) || see(DO) || see(TRY) ||
                see(SWITCH) || see(RETURN) || see(THROW) || see(BREAK) || see(CONTINUE) ||
                see(SEMI)) {
            return true;
        }
