...
class JUnaryPlusOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        operand = operand.analyze(context);
        operand.type().mustMatchOneOf(line(), Type.INT, Type.DOUBLE, Type.LONG);
        type = operand.type();
        return this;
    }
...
/**
 * The AST node for a bitwise unary complement (~) expression.
 */
class JComplementOp extends JUnaryExpression {
    /**
     * Constructs an AST node for a bitwise unary complement expression.
     *
     * @param line    line in which the bitwise unary complement expression occurs in the source
     *                file.
     * @param operand the operand.
     */
